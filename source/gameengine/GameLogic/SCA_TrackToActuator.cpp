...
SCA_TrackToActuator::SCA_TrackToActuator(
    SCA_IObject *gameobj, SCA_IObject *ob, int time, bool allow3D, int trackflag, int upflag)
...
static MT_Matrix3x3 vectomat(MT_Vector3 vec, short axis, short upflag, short threedimup)
{
  ...
  MT_Vector3 z(
      MT_Scalar(0.0f), MT_Scalar(0.0f), MT_Scalar(1.0f)); /* world Z axis is the global up axis */
  MT_Vector3 proj;
  MT_Vector3 right;
  MT_Scalar mul;
  int right_index;
...
bool SCA_TrackToActuator::Update(double curtime)
{
  ...
  if (bNegativeEvent) {
    ...
    MT_Vector3 dir = curobj->NodeGetWorldPosition() -
                     ((KX_GameObject *)m_object)->NodeGetWorldPosition();
    MT_Matrix3x3 mat;
    MT_Matrix3x3 oldmat;
                                           ...
                                           PyObject *value)
{
  ...
  if (!ConvertPythonToGameObject(actuator->GetLogicManager(),
                                 ...
                                 &gameobj,
                                 true,
                                 "actuator.object = value: SCA_TrackToActuator"))
