...
/** \file gameengine/VideoTexture/DeckLink.cpp
 *  \ingroup bgevideotex
 */
...
static struct {
  const char *name;
  BMDDisplayMode mode;
} sModeStringTab[] = {{"NTSC", bmdModeNTSC},
...
} sFormatStringTab[] = {{"8BitYUV", bmdFormat8BitYUV},
...
HRESULT decklink_ReadDisplayMode(const char *format, size_t len, BMDDisplayMode *displayMode)
{
  ...
  for (i = 0; sModeStringTab[i].name != nullptr; i++) {
    if (strlen(sModeStringTab[i].name) == len && !strncmp(sModeStringTab[i].name, format, len)) {
  ...
  *displayMode = (BMDDisplayMode)((((uint32_t)format[0]) << 24) + (((uint32_t)format[1]) << 16) +
                                  (((uint32_t)format[2]) << 8) + ((uint32_t)format[3]));
  return S_OK;
}
...
HRESULT decklink_ReadPixelFormat(const char *format, size_t len, BMDPixelFormat *pixelFormat)
{
  ...
  // assume the user entered directly the mode value as a 4 char string
  *pixelFormat = (BMDPixelFormat)((((uint32_t)format[0]) << 24) + (((uint32_t)format[1]) << 16) +
                                  (((uint32_t)format[2]) << 8) + ((uint32_t)format[3]));
  return S_OK;
}
...
static void decklink_ConvImage(uint32_t *dest,
                               const short *destSize,
...
ExpDesc DeckLinkBadFormatDesc(DeckLinkBadFormat,
                              "Invalid or unsupported output format, use <mode>[/3D]");
ExpDesc DeckLinkInternalErrorDesc(DeckLinkInternalError,
                                  "DeckLink API internal error, please report");
  ...
  try {
    ...
    while (pDisplayModeIterator->Next(&pDisplayMode) == S_OK) {
      if (pDisplayMode->GetDisplayMode() == self->mDisplayMode &&
        ...
        if (self->mDLOutput->DoesSupportVideoMode(
                self->mDisplayMode, bmdFormat8BitBGRA, outputFlags, &support, nullptr) != S_OK ||
    ...
    if (self->mDLOutput->CreateVideoFrame(self->mSize[0],
                                          self->mSize[1],
                                          self->mSize[0] * 4,
                                          bmdFormat8BitBGRA,
                                          bmdFrameFlagFlipVertical,
                                          &self->mLeftFrame) != S_OK)
      THRWEXCP(DeckLinkInternalError, S_OK);
  ...
  if (engine->GetClockTime() != self->m_lastClock) {
    ...
    try {
      ...
      if (leftEye) {
        ...
        if (self->mUse3D) {
          DeckLink3DFrameWrapper frame3D((IDeckLinkVideoFrame *)self->mLeftFrame,
...
static PyGetSetDef decklinkGetSets[] = {
    {(char *)"source",
     (getter)DeckLink_getSource,
     (setter)DeckLink_setSource,
     (char *)"source of decklink (left eye)",
     nullptr},
    {(char *)"right",
     (getter)DeckLink_getRight,
     (setter)DeckLink_setRight,
     (char *)"source of decklink (right eye)",
     nullptr},
    {(char *)"keying",
     (getter)DeckLink_getKeying,
     (setter)DeckLink_setKeying,
     (char *)"whether keying is enabled (frame is alpha-composited with passthrough output)",
     nullptr},
    {(char *)"level",
     (getter)DeckLink_getLevel,
     (setter)DeckLink_setLevel,
     (char *)"change the level of keying (overall alpha level of key frame, 0 to 255)",
     nullptr},
    {(char *)"extend",
     (getter)DeckLink_getExtend,
     (setter)DeckLink_setExtend,
     (char *)"whether image should stretched to fit frame",
     nullptr},
    {nullptr}};

// class DeckLink declaration
PyTypeObject DeckLinkType = {
    PyVarObject_HEAD_INIT(nullptr, 0) "VideoTexture.DeckLink", /*tp_name*/
