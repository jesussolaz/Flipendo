...
class FilterRGB24 : public FilterBase {
 ...
 protected:
  ...
  virtual unsigned int filter(
      unsigned char *src, short x, short y, short *size, unsigned int pixSize, unsigned int val)
  {
    VT_RGBA(val, src[0], src[1], src[2], 0xFF);
    return val;
  }
};
...
class FilterBGR24 : public FilterBase {
 ...
 protected:
  ...
  virtual unsigned int filter(
      unsigned char *src, short x, short y, short *size, unsigned int pixSize, unsigned int val)
  {
    VT_RGBA(val, src[2], src[1], src[0], 0xFF);
    return val;
  }
};
...
class FilterZZZA : public FilterBase {
 ...
 protected:
  ...
  virtual unsigned int filter(
      float *src, short x, short y, short *size, unsigned int pixSize, unsigned int val)
  {
...
class FilterYV12 : public FilterBase {
 public:
  /// constructor
  FilterYV12(void) : m_buffV(nullptr), m_buffU(nullptr), m_pitchUV(0)
  {
  }
  /// destructor
 ...
 protected:
  ...
  {
    return interpol(interpolV(src - 1), interpolV(src), interpolV(src + 1), interpolV(src + 2));
  }
  ...
  {
    return interpol(*interParA(src, x, size, 1),
                    *src,
                    *interParC(src, x, size, 1),
                    *interParD(src, x, size, 1));
  }

  /// vertical interpolation on edges
  int interpolEV(unsigned char *src, short y, short size)
  {
    return interpol(*interParA(src, y, size, m_pitchUV),
                    *src,
                    *interParC(src, y, size, m_pitchUV),
                    *interParD(src, y, size, m_pitchUV));
  }
  ...
  int interpolEVH(unsigned char *src, short x, short y, short *size)
  {
    return interpol(interpolEV(interParA(src, x, size[0], 1), y, size[1]),
                    interpolEV(src, y, size[1]),
                    interpolEV(interParC(src, x, size[0], 1), y, size[1]),
