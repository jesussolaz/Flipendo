...
bool check(bContext *C, const char *baseline_path)
{
  ...
  if (fb == nullptr) {
    fprintf(stderr, "fl-check-mesh-ops: no se pudo leer la linea base '%s'\n", baseline_path);
    return false;
  }
  FILE *fa = fopen(actual_path, "r");
  if (fa == nullptr) {
    fclose(fb);
    fprintf(stderr, "fl-check-mesh-ops: no se pudo releer '%s'\n", actual_path);
    return false;
  }

  char line_b[1024], line_a[1024];
  int line_no = 0;
  int total_values = 0, same_values = 0;
  int total_lines = 0, same_lines = 0;
  int shown = 0;
  char current_case[256] = "(sin caso)";
  /* Diferencias por caso, indexadas por el orden de aparicion. */
  ...
  while (true) {
    ...
    if (strncmp(sb, "case=", 5) == 0) {
      BLI_strncpy(current_case, sb, sizeof(current_case));
      if (char *nl = strchr(current_case, '\n')) {
        *nl = '\0';
      }
      case_labels.append(current_case);
      case_total.append(0);
      case_same.append(0);
    }

    total_lines++;
    /* Solo las lineas de datos (las que empiezan por dos espacios y un digito) cuentan
     * como "valores": el resto son cabeceras. */
    ...
    if (rb != nullptr && ra != nullptr && STREQ(sb, sa)) {
      ...
      fprintf(stderr,
              "fl-check-mesh-ops: linea %d distinta\n  base: %s  real: %s",
              line_no,
              sb,
              sa);
      shown++;
    }
    if (is_value && !case_total.is_empty()) {
      case_total.last()++;
    }
  }
  ...
  for (const int i : case_labels.index_range()) {
    fprintf(stderr,
            "fl-check-mesh-ops: %-56s %d/%d elementos identicos\n",
            case_labels[i].c_str(),
            case_same[i],
            case_total[i]);
  }
  fprintf(stderr,
          "fl-check-mesh-ops: TOTAL %d/%d elementos de color identicos, %d/%d lineas\n",
          same_values,
          total_values,
          same_lines,
          total_lines);
