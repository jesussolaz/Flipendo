...
static const char arg_handle_fl_dump_keymap_doc[] =
    "<filepath>\n"
    "\tVuelca el mapa de teclado activo a un fichero de texto y sale.\n"
    "\tSirve para comparar configuraciones de teclado entre builds "
    "(migracion del keymap a C++).";
static int arg_handle_fl_dump_keymap(int argc, const char **argv, void *data)
{
  bContext *C = static_cast<bContext *>(data);
  if (argc > 1) {
...
static const char arg_handle_fl_dump_tool_queries_doc[] =
    "<filepath>\n"
    "\tVuelca etiqueta, grupo y descripcion de cada herramienta y sale (modo grafico).\n"
    "\tSe compara con tests/flipendo/toolsystem/queries-python.txt.";
static int arg_handle_fl_dump_tool_queries(int argc, const char **argv, void *data)
{
  bContext *C = static_cast<bContext *>(data);
  if (argc > 1) {
