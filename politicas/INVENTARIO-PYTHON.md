...
|---|---:|---|---|---|---:|---|---|
| `wm.py` | 3.015 | 24 clases: los 7 `wm.context_*` que faltan, `batch_rename`, `properties_edit`, `doc_view*`, `url_open*`, `path_open`, `sysinfo`, `blenderplayer_start`, `owner_*`, 4 menús (`WM_MT_splash*`, `WM_MT_region_toggle_pie`) | Sí. **12 de sus clases las nombra el C++ por cadena** | **MIG** `wmOperatorType` + `MenuType`. Carril B | 55–85 | `wm_context_ops.cc` (existe, 588 líneas, 11 operadores) | **Alto**: `wm_splash_screen.cc:348,355,476` busca `WM_MT_splash*` — sin ellos no hay pantalla de bienvenida; `screen_ops.cc:6813` registra `WM_OT_drop_blend_file` como manejador de soltar ficheros |
